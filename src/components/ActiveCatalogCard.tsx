...
export const ActiveCatalogCard: React.FC<ActiveCatalogCardProps> = ({
  ...
  return (
    <div
      className={`bg-surface border border-line rounded-2xl p-6 space-y-4 shadow-lg text-fg transition-all duration-200 hover:border-primary/40 ${className}`}
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-line pb-3.5">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-base font-bold text-fg tracking-[-0.3px]">
        ...
        {totalCount > 0 && (
          <button
            ...
            className="text-xs text-fg-muted hover:text-primary-soft hover:underline cursor-pointer flex items-center gap-1 font-semibold transition-colors"
          >
      ...
      {totalCount === 0 ? (
        <div className="bg-canvas border border-line rounded-card p-8 md:p-12 text-center flex flex-col items-center justify-center relative overflow-hidden shadow-md">
          {/* Ambient Glows */}
          <div className="absolute -top-16 -right-16 w-52 h-52 bg-warning/10 rounded-full blur-3xl pointer-events-none" />
          <div className="absolute -bottom-16 -left-16 w-52 h-52 bg-primary/15 rounded-full blur-3xl pointer-events-none" />

          {/* Animated Spinning Vinyl Record */}
          <div className="relative mb-6 group">
            {/* Spinning Vinyl Disc */}
            <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-full bg-[#111110] border-4 border-line flex items-center justify-center shadow-xl relative animate-spin-slow transition-transform duration-300 group-hover:scale-105">
              ...
              <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-gradient-to-tr from-[#8B5CF6] via-[#EC4899] to-[#F59E0B] border-2 border-stone-900 flex flex-col items-center justify-center text-white shadow-inner relative z-10">
                ...
                <span className="text-2xs font-black tracking-tighter uppercase font-mono mt-[-2px]">
                  33 RPM
                </span>
            ...
            <div
              className="absolute -top-1.5 -right-1.5 w-8 h-8 rounded-full bg-primary text-white flex items-center justify-center shadow-md z-20"
            >
          ...
          <h3 className="text-xl sm:text-2xl font-extrabold text-fg tracking-[-0.8px]">
            Tu catálogo está esperando tu primer hit underground
          </h3>
          <p className="text-xs sm:text-sm text-fg-muted max-w-lg mt-2.5 leading-relaxed font-normal">
          ...
          <button
            ...
            className="mt-6 inline-flex items-center gap-2 bg-gradient-to-r from-[#8B5CF6] to-[#EC4899] text-white hover:opacity-90 active:scale-98 text-xs sm:text-sm font-bold px-6 py-3 rounded-lg transition-all cursor-pointer shadow-[0_0_15px_rgba(139,92,246,0.4)]"
          >
            <Mic2 className="w-4 h-4 text-white" />
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            {displaySongs.map((song, i) => {
              ...
              return (
                <div
                  key={song.id}
                  className="p-4 rounded-xl bg-canvas border border-line hover:border-primary/50 transition-all duration-200 flex flex-col justify-between gap-3 shadow-xs group"
                >
                  {/* Top Row: Rank, Title, Genre Badge & Date */}
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3 min-w-0">
                      <span className="text-xs font-bold text-fg-muted w-6 h-6 rounded-md bg-surface border border-line flex items-center justify-center shrink-0">
                        #{i + 1}
                      </span>
                      <div className="min-w-0 space-y-1">
                        <h4
                          className="text-sm font-bold text-fg tracking-tight truncate flex items-center gap-1.5"
                          title={song.title}
                        >
                          {song.title}
                        </h4>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span
                            className={`text-2xs font-bold px-2 py-0.5 rounded-sm border ${genreTheme.badgeBg} ${genreTheme.badgeText} ${genreTheme.badgeBorder}`}
                          >
                            {genreName}
                          </span>
                          <span
                            className="text-2xs text-fg-muted font-mono flex items-center gap-1"
                            title={fullDateStr}
                          >
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap pt-0.5">
                    ...
                    <span
                      className="text-2xs font-medium bg-surface border border-line text-fg-muted px-2 py-0.5 rounded-sm flex items-center gap-1 truncate max-w-[150px] shadow-xs"
                    ...
                    {song.musicVideo && (
                      <span
                        className="text-2xs font-bold bg-cyan-950/70 text-cyan-300 border border-cyan-500/50 px-2 py-0.5 rounded-sm flex items-center gap-1 shadow-xs"
                    ...
                    {song.wentViral && (
                      <span className="text-2xs font-bold bg-gradient-to-r from-fuchsia-500 to-pink-500 text-white px-2 py-0.5 rounded-sm flex items-center gap-1 shadow-xs">
                    ...
                    {isHitTop10 && (
                      <span className="text-2xs font-bold bg-gradient-to-r from-orange-500 to-amber-500 text-white px-2 py-0.5 rounded-sm flex items-center gap-1 shadow-xs">
                    ...
                    {song.isClassic && (
                      <span className="text-2xs font-bold bg-gradient-to-r from-amber-400 to-yellow-500 text-stone-950 px-2 py-0.5 rounded-sm flex items-center gap-1 shadow-xs">
                    ...
                    {peakGlobal && (
                      <span className="text-2xs font-bold bg-indigo-950/60 text-indigo-300 border border-indigo-500/40 px-2 py-0.5 rounded-sm shadow-xs">
                        Peak #{peakGlobal}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-line text-xs">
                    <div>
                      <span className="text-2xs text-fg-muted uppercase block font-medium">
                        Mensual
                      </span>
                      <span className="text-xs font-bold text-emerald-400">
                        {song.streamsLastMonth.toLocaleString()}{' '}
                        <span className="font-normal text-2xs text-fg-muted">/mes</span>
                      </span>
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-xs font-bold text-primary-soft font-mono">
                        {formatCompactNumber(song.streamsTotal)}{' '}
                        <span className="font-normal text-2xs text-fg-muted">tot.</span>
                      </span>
                    </div>
                  </div>
                </div>
          ...
          {/* Quick Action Footer */}
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-3 bg-canvas border border-line rounded-control text-xs text-fg-muted">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-semibold text-info hover:text-[#38BDF8] hover:underline cursor-pointer whitespace-nowrap"
            >
